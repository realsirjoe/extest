...
func main() {
	...
	mux.HandleFunc("/api/product/", func(w http.ResponseWriter, r *http.Request) {
		rest := strings.TrimPrefix(r.URL.Path, "/api/product/")
		if rest == "" || rest == r.URL.Path {
			http.Error(w, "missing product id", http.StatusBadRequest)
			return
		}

		if strings.HasSuffix(rest, "/similar") {
			id := strings.TrimSuffix(rest, "/similar")
	...
	log.Printf("easy-server listening on %s (table=%s id=%s)", *addr, table, *idCol)
  ...
  <script>
    (function () {
      ...
      fetch("/api/product/" + encodeURIComponent(productId) + "/similar", { headers: { "Accept": "application/json" } })
        ...
        .catch(function () {
          statusEl.textContent = "Could not load suggestions right now.";
        });
    })();
  </script>
</body>
