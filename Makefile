...
.PHONY: serve-easy test build-easy-server build-compare-csv build-process-dm-products build-shuffle-csv build-all
...
build-process-dm-products:
	mkdir -p $(BIN_DIR)
	GOCACHE=/tmp/gocache GOWORK=off go build -o $(BIN_DIR)/process-dm-products ./cmd/process-dm-products

build-shuffle-csv:
	mkdir -p $(BIN_DIR)
	GOCACHE=/tmp/gocache GOWORK=off go build -o $(BIN_DIR)/shuffle-csv ./cmd/shuffle-csv

build-all: build-easy-server build-compare-csv build-process-dm-products build-shuffle-csv
